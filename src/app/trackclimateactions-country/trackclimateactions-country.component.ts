...
export class TrackclimateactionsCountryComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.yearList = await this.assYearProxy
      .getAssessmentYearsListInTrackCA()
      .toPromise();
        ...
        this.serviceProxy
          ...
          .subscribe(async (res) => {
            if (res.data.length == 0) {
              ...
              const maxNumber = res.data[0].flag;
              this.flag = maxNumber + 1;
            }

            this.climateactionserviceproxy
              .getTrackClimateActionsDetails(
                pageNumber,
                this.rows,
                filtertext,
                this.selectedYear,
              ...
              .subscribe((a) => {
                ...
                for (const project of this.trackClimateActions) {
                  const activeproject2: activeprojectsave = {
                    projectId: 0,
                    Name: '',
                    Des: '',
                    Obj: '',
                  ...
                  activeproject1.startYear_of_implementation = Number(
                    project.proposeDateofCommence.toString().split('-')[0],
                  );
                  ...
                  if (project.assessment[0].assessmentType == 'Ex-ante') {
                    activeproject1.expected =
                      project.assessment[0]?.assessementResult[0]?.totalEmission;
                  } else {
                    activeproject1.achieved =
                      project.assessment[0]?.assessementResult[0]?.totalEmission;
                  }
                  ...
                  activeproject2.startYear =
                    activeproject1.startYear_of_implementation;
                  activeproject2.implementing =
                    activeproject1.implementing_entity_or_entities;
  ...
  download() {
    for (const item of this.activeprojects) {
      const obj: excelTrackCas = {
        Name: '',
        Description: '',
        Objectives: '',
        Type_of_Instrument: '',
      ...
      obj.Implementing_Entity_or_Entities =
        item.implementing_entity_or_entities;
  ...
  onNDCChange(event: any) {
    const ndcIdArray: any[] = [];
    for (const ndcObj of event) {
  ...
  publish() {
    ...
    this.activeprojectson1.forEach((project, index) => {
      const trackca: TrackcaEntity = new TrackcaEntity();
